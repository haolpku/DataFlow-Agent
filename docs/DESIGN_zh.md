...
我们把整个过程拆成四个独立的算子(operator),像四块积木一样拼起来:
...
1. **采集 ≠ 数据。** 价值在于打分和筛选,只留高质量轨迹。
2. **算子不认沙箱。** 一切通过抽象接口解耦,换后端不改业务代码。
3. **便宜的先上。** 规则过滤器(不花钱)打头阵,大模型裁判(花钱)殿后。

---

## 8. 路线图(未来工作)

- **多模态探索器**:支持 GUI / 虚拟机的图像观察。
- **Refiner(修复器)**:把低分轨迹自动重写/修复,补齐"生成→评估→过滤→修复"闭环。
