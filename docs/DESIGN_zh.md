...
我们把整个过程拆成六个独立的算子(operator),像积木一样拼起来:
...
```
   ...
   任务列表  ───►   │  生成器      │  让模型在沙箱里探索,产出轨迹
                    ...
                    └──────┬──────┘
                           ...
                           ▼
                    ┌─────────────┐
                    │ 选择器/过滤器 │  选择器:从一堆里挑 top-N 且多样(不调模型)
                    │Selector/Filter│  过滤器:按规则丢掉不合格的(不调模型)
                    └──────┬──────┘
                           │ 高质量轨迹
                           ▼
                    ┌─────────────┐
...
> 注:选择器、过滤器、修复器是三种处理轨迹的策略,可按需组合——
> 「选器」从探索树的多条路径里挑精华,「过滤器」快速丢掉明显不合格的,
> 「修复器」尽量救回可挽救的。想要又快又纯净就用过滤器,想从树里选多样样本就用选择器,想尽量不浪费数据就加修复器。
